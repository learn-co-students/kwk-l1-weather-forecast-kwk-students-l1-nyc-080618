...
def city_weather_info(weather)
  weather.each do |info, value|
    puts "The #{info} is #{value}."
  end
end
...
def convert_temp(weather)
  temperature = weather[:temperature_farenheit]
  celsius = (temperature - 32) * 5 / 9.0 #One of the numbers needs to be float in division to get a float
  rounded_celsius = (celsius * 100).round / 100.0
